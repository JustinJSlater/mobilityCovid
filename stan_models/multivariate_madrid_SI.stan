...
transformed parameters{
  ...
  for (t in 1:T){
    ...
    log_prob[t] = test_effect*log(num_tests[t]);
  }
}
...
model{
  for (i in 1:I){
    for (t in 1:T){
      target += poisson_lpmf(y[i,t]|exp(log_prob[t])*mu[i,t]);
    }
  }
...
generated quantities{
  ...
  real<lower=0> inf_per_trip_agg_noCity[T];
  ...
  for (i in 1:I){
    ...
    for (t in 1:T){
      ...
      log_lik[i,t] = poisson_lpmf(y[i,t]|exp(log_prob[t])*mu[i,t]);
    }
  ...
  for (t in 1:T){
    inf_per_trip_agg[t] = sum(EP_move[,t])/
    sum(rho*(mob_sum1[,t] + mob_sum2[,t] + mob_sum3[,t]) + (1-rho)*(mob_sum4[,t] + mob_sum5[,t]) );
    inf_per_trip_agg_noCity[t] = sum(EP_move[2:I,t])/
    sum(rho*(mob_sum1[2:I,t] + mob_sum2[2:I,t] + mob_sum3[2:I,t]) + (1-rho)*(mob_sum4[2:I,t] + mob_sum5[2:I,t]) );
