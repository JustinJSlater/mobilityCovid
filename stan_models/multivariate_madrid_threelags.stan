...
model{
  ...
  alpha_en ~ normal(0,1);
  alpha_ar~normal(0,2);
  alpha_move[1,] ~ normal(0,0.1);
  alpha_move[2,] ~ normal(0,0.1);
  alpha_move[3,] ~ normal(0,0.1);

  alpha_spat ~ normal(0,500);
  
  test_effect ~ normal(0,0.5);
}
...
generated quantities{
  ...
  for (t in 1:T){
    ...
    inf_per_trip_agg_noCity[t] = sum(EP_move[2:I,t])/sum(mob_sum1[2:I,t] + mob_sum2[2:I,t] + mob_sum3[2:I,t]);
  }  
}
